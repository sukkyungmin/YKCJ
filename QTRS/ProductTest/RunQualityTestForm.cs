...
namespace QTRS.ProductTest
{
    public partial class RunQualityTestForm : Form
    {
        ...
        private bool _absorbeAmountMode = false;
        private AbsorbedAmountTestForm _absorbedAmountTestForm = null;
        public string _absorbedAmountData = ""; 

        public enum eProductQtTestResultList
        {
        ...
        public RunQualityTestForm(AddProductTestForm parent)
        {
            InitializeComponent();
            _parent = parent;
            _currentTestId = _parent._currentTestId;

            string[] productname = _parent._productname.Split('|');

            if(productname.Count() > 1)
            {
                ProductName.Text = string.Format("CODE : {0}       NAME : {1}", productname[0], productname[1]);
            }
            else
            {
                ProductName.Text = string.Format("{0}", _parent._productname);
            }
        ...
        private void OpenScalePort()
        {
            if (this.ohausSerialPort.IsOpen == false)
            {
                if (Global.comNumber != "")
                {
                    this.ohausSerialPort.PortName = Global.comNumber;
                    this.ohausSerialPort.BaudRate = 2400;
                    this.ohausSerialPort.DataBits = 8;
                    this.ohausSerialPort.StopBits = StopBits.One;
                    this.ohausSerialPort.Parity = Parity.None;

                    try
                    {
                        if (this.ohausSerialPort.IsOpen == false)
                        {
                            this.ohausSerialPort.Open();
                        }
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(Global.comNumber + " 포트를 열 수 없습니다. 저울과 연결된 포트인지 확인해 주십시오.");

                        QTRS.ProductTest.SerialPortSetup form = new ProductTest.SerialPortSetup(this.ohausSerialPort) { StartPosition = FormStartPosition.CenterParent };
                        form.ShowDialog();
                    }
                }
                else
                {
                    MessageBox.Show("선택된 포트가 없습니다. 저울과 연결된 포트를 확인해 주십시오.");
        ...
        private void InitTestMethodMap()
        {
            ...
            foreach (DataRow dataRow in dataSet.Tables[0].Rows)
            {
                ...
                if (testItemId == "1") // 성상
                {
                    query = string.Format("EXEC GetPropertyValue '{0}'", _currentTestId);
                    string property = DbHelper.GetValue(query, "property", "");
                    testStandard = property;
                    testScore = property;

                    compatibilityOx = (testScore == "") ? "부적합" : "적합"; 
                }
                else if (testItemId == "3") // 질량
                {
                    testScore = ""; 
                ...
                else if (testItemId == "8") //  흡수량
                {
                    query = string.Format("EXEC GetStandardOnAbsorbedAmountValue '{0}'", _currentTestId);
                    //string query = string.Format("SELECT PM.standardOnAbsorbedAmount FROM ProductTests PT INNER JOIN ProductMasters PM ON PT.productCode = PM.productCode WHERE PT.idx = '{0}'", _currentTestId);
                    string standardOnAbsorbedAmount = DbHelper.GetValue(query, "standardOnAbsorbedAmount", "");

                    testStandard = standardOnAbsorbedAmount + "배";
                    testScore = ""; 


                    if (testScore == "")
                        compatibilityOx = "부적합";
                    else if (Utils.GetDoubleValue(testScore) >= Utils.GetDoubleValue(testStandard))
                        compatibilityOx = "적합";
                    else
                        compatibilityOx = "부적합";
                }
                ...
                productQtDataGridView.Rows.Add(
                    testItem,
                    testMethod,
                    testStandard,
                    testScore,
        ...
        private void InitProductTestDataGridView()
        {
            ...
            dataGridView.Columns[(int)eProductQtTestResultList.testMethod].Visible = false;
            dataGridView.Columns[(int)eProductQtTestResultList.testItemId].Visible = false;
            ...
            // Each column style 
            dataGridView.Columns[(int)eProductQtTestResultList.testItem].Width = 150;
            dataGridView.Columns[(int)eProductQtTestResultList.testScore].Width = 400;
            dataGridView.Columns[(int)eProductQtTestResultList.compatibilityOx].Width = 80;

            int tempWidth = dataGridView.Columns[(int)eProductQtTestResultList.testItem].Width +
                dataGridView.Columns[(int)eProductQtTestResultList.testScore].Width +
                 dataGridView.Columns[(int)eProductQtTestResultList.compatibilityOx].Width;

            //dataGridView.Columns[(int)eProductQtTestResultList.testMethod].Width = (dataGridView.Width - tempWidth) / 2;
            dataGridView.Columns[(int)eProductQtTestResultList.testStandard].Width = (dataGridView.Width - tempWidth);


            // Each column style 
            for (int i = (int)eProductQtTestResultList.testItem; i <= (int)eProductQtTestResultList.compatibilityOx; i++)
            {
                if (i == (int)eProductQtTestResultList.testScore)
        ...
        private void addDataButton_Click(object sender, EventArgs e)
        {
            ...
            {
                // 질량 10번 체크
                for (int i = 0; i < productQtDataGridView.Rows.Count; i++)
                {
                    if("3" == productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testItemId].Value.ToString())
                    {
                        string testScore = Utils.GetString(productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testScore].Value);
                        if (testScore.Trim().Split('/').Length != 10)
                        {
                            MessageBox.Show("질량을 10번 입력해 주십시오.");
                            return; 
                        }
                    }
                }
        ...
        private void AddTest(SqlCommand command)
        {
            try
            {
                ...
                command.CommandText = string.Format("EXEC DeleteProductQtTestResults '{0}'", _currentTestId);
                ...
                for (int i = 0; i < productQtDataGridView.Rows.Count; i++)
                {
                    string key = productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.idx].Value.ToString();

                    string testItemId = productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testItemId].Value.ToString();
                    string testItemName = productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testItem].Value.ToString();
                    string testMethod = productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testMethod].Value.ToString();
                    string testStandard = productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testStandard].Value.ToString();
                    string testScore = productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testScore].Value.ToString();
                    string compatibilityOx = productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.compatibilityOx].Value.ToString();

                    if(testItemId == "3")
                    {
                        testScore = testScore.Trim();
                        testScore = testScore.Replace('/', Global.DB_VALUE_SEPARATOR.ToArray()[0]);
                    }
                    else if (testItemId == "8")
                    {
                        testScore = _absorbedAmountData.Trim();
                        testScore = testScore.Replace(' ', Global.DB_VALUE_SEPARATOR.ToArray()[0]);
                    ...
                    string query = "EXEC InsertProductQtTestResultItem '{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}' ";
                ...
                if (totalCompatibilityCount != 0 && totalCompatibilityCount == productQtDataGridView.Rows.Count)
                ...
                for (int di = 0; di < dataGridViews.Length; di++)
                {
                    ...
                    for (int fi = 0; fi < dataGridViews[di].Rows.Count; fi++)
                    {
                        ...
                        if (id != "")
                        {
                            ...
                            //command.CommandText = string.Format("EXEC UpdateTestAttachmentItem '{0}', '{1}', '{2}'", fileMemo, Global.FILETYPE_PRODUCT_Q, id);
                        ...
                        else
                        {
                            ...
                            if (fileData.Value != null)
                            {
                                command.CommandText = string.Format("EXEC InsertTestAttachmentItem '{0}', '{1}', '{2}', '{3}', @fileData, '{4}' ",
                                    _currentTestId, fileName, fileMemo, fileType, Global.FILETYPE_PRODUCT_Q);
        ...
        private bool DownloadFiles(string downloadFilePath)
        {
            for (int i = 0; i < fileDataGridView.Rows.Count; i++)
            {
                if ((bool)fileDataGridView.Rows[i].Cells[(int)eFileDataList.CHECKBOX].Value == true)
                {
                    ...
                    if (id != "") // 다운로드
                    {
                        DataSet dataSet = DbHelper.SelectQuery(string.Format("EXEC SelectTestAttachmentItem '{0}', '{1}'", id, Global.FILETYPE_PRODUCT_Q));
                        if (dataSet == null || dataSet.Tables.Count == 0)
                        {
        ...
        private void deleteFileButton_Click(object sender, EventArgs e)
        {
            ...
            string query = string.Format("EXEC DeleteTestAttachment '{0}', '{1}'", ids, Global.FILETYPE_PRODUCT_Q);
            long retVal = DbHelper.ExecuteNonQuery(query);
            if (retVal != -1)
            {
                for (int i = 0; i < dataGridView.Rows.Count; i++)
        ...
        private void exportButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                */
                ...
                //{
                // 라벨 입력
                worksheet.Cells[currentRow, 1] = "완제품 품질관리 검사";
        ...
        private void SelectProductQtTestResult()
        {
            ...
            foreach (DataRow dataRow in dataSet.Tables[0].Rows)
            {
                ...
                if (testItemId == "3" ) // 질량
                {
                    ...
                    testScore = testScore.Replace(Global.DB_VALUE_SEPARATOR.ToCharArray()[0], '/');
                }
                else if(testItemId == "8") // 흡수량
                {
                    if (testScore == "")
                    ...
                    _absorbedAmountData = testScore;
                    string[] testScoreArray = _absorbedAmountData.Trim().Split(Global.DB_VALUE_SEPARATOR.ToCharArray()[0]);
                    // 흡수량 1
                    ...
                    testScore = string.Format("{0:0.0} ", absorbedAmountAverage) + "배";

                }

                for (int i=0; i< productQtDataGridView.Rows.Count; i++)
                {
                    if(testItemId == productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testItemId].Value.ToString())
                    {
                        productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.testScore].Value = testScore;
                        productQtDataGridView.Rows[i].Cells[(int)eProductQtTestResultList.compatibilityOx].Value = compatibilityOx; 
                    }
                }
            }
        }
        ...
        private void SetTestFormatFiles()
        {
            ...
            for (int di = 0; di < dataGridViews.Length; di++)
            {
                ...
                DataSet dataSet = DbHelper.SelectQuery(string.Format("EXEC SelectTestAttachmentList '{0}', '{1}', '{2}'", _currentTestId, fileType, Global.FILETYPE_PRODUCT_Q));
                if (dataSet == null || dataSet.Tables.Count == 0)
                {
                    MessageBox.Show("파일 정보를 가져올 수 없습니다.");
                    break;
        ...
        private void testItemComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if(_testMethodMap[key].testItemId == "1") // 성상
            {
                string query = string.Format("EXEC GetPropertyValue '{0}'", _currentTestId);
                string property = DbHelper.GetValue(query, "property", "");
                testStandardTextBox.Text = property;
                testScoreTextBox.Text = property;
            }
            else if (_testMethodMap[key].testItemId == "3") // 질량
            {
                testStandardTextBox.Text = _testMethodMap[key].testStandard;
                testScoreTextBox.Text = ""; // _testMethodMap[key].testScore;
                testScoreTextBox.Enabled = false;
            }
            else if (_testMethodMap[key].testItemId == "8") //  흡수량
            {
                //string query = string.Format("EXEC GetStandardOnAbsorbedAmountValue '{0}'", _currentTestId);
                string query = string.Format("SELECT PM.standardOnAbsorbedAmount FROM ProductTests PT INNER JOIN ProductMasters PM ON PT.productCode = PM.productCode WHERE PT.idx = '{0}'", _currentTestId);
                string standardOnAbsorbedAmount = DbHelper.GetValue(query, "standardOnAbsorbedAmount", "");

                testStandardTextBox.Text = standardOnAbsorbedAmount + "배";
                testScoreTextBox.Text = ""; // _testMethodMap[key].testScore;
                testScoreTextBox.Enabled = false;

                _absorbeAmountMode = true;
                OpenProductTestForm("ADD");
                 _absorbeAmountMode = false; 
        ...
        */ 
        ...
        private void ohausSerialPort_DataReceivedHandler(object sender, EventArgs e)
        {
            ...
            AddSerialPortData(data); 
           
        }

        /**
         * 여기서 받는 데이터를 질량 이다. 
         * 흡수량은 별도의 창에서 데이터를 받는다.
         */
        public void AddSerialPortData(string data)
        {
            _serialData += data;

            if (_serialData.IndexOf(Environment.NewLine) != -1)
            {
                _serialData = Utils.GetRealNumberString(_serialData);

                if (_absorbeAmountMode == true)
                {
                    //_absorbedAmountTestForm.SetAbsorbedAmountData(_serialData);
                    _absorbedAmountTestForm.AddAbsorbedAmountData(_serialData);
                }
                else
                {
                    string testItemId = productQtDataGridView.SelectedRows[0].Cells[(int)eProductQtTestResultList.testItemId].Value.ToString(); 

                    if ("3" == testItemId)
                    {
                        string testScore = ""; 
                        if (productQtDataGridView.SelectedRows[0].Cells[(int)eProductQtTestResultList.testScore].Value != null)
                            testScore = productQtDataGridView.SelectedRows[0].Cells[(int)eProductQtTestResultList.testScore].Value.ToString();

                        if (testScore.Trim().Split('/').Length == 10) 
                        {
                            _serialData = "";
                            MessageBox.Show("10개의 데이터가 채워졌습니다.");
                            return;
                        }

                        if(testScore == "")
                        {
                            testScore += Utils.GetRealNumberString(_serialData);
                        }
                        else
                        {
                            testScore += "/" + Utils.GetRealNumberString(_serialData);
                        ...
                        productQtDataGridView.SelectedRows[0].Cells[(int)eProductQtTestResultList.testScore].Value = testScore;
                    }
                }
        ...
        private void RunQualityTestForm_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            AddSerialPortData("2.2\r\n");
        }

        private void SetTotalCompatibilityOx(SqlCommand command)
        {
            command.CommandText = string.Format("EXEC UpdateProductTestJudgingResult '{0}', '{1}'", _currentTestId, _totalCompatibilityOx == false ? 0 : 1);
            int retVal = command.ExecuteNonQuery();
            if (retVal < 1)
                throw new Exception("Test를 저장할 수 없습니다.");
            else
            ...
            int selectedRowIndex = _parent._parent.productTestDataGridView.SelectedRows[0].Index;
            _parent._parent.productTestDataGridView.Rows[selectedRowIndex].Cells[(int)QTRS.ProductTest.ProductTestControl.eProductTestList.judgingResult].Value = _totalCompatibilityOx == true ? "적합" : "부적합"; 

        }

        private void OpenProductTestForm(string saveMode = "ADD")
        {
            _absorbedAmountTestForm = new AbsorbedAmountTestForm(this);

            if (saveMode == "UPDATE")
                _absorbedAmountTestForm._absorbedAmountData = _absorbedAmountData;

            _absorbedAmountTestForm.SetSaveMode(saveMode);
            _absorbedAmountTestForm.StartPosition = FormStartPosition.CenterParent;
            _absorbedAmountTestForm.ShowDialog();
            _absorbedAmountTestForm.Dispose();
            _absorbedAmountTestForm = null;
        }

        private void productQtDataGridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex < 0 || e.RowIndex < 0)
                return;

            if (productQtDataGridView.SelectedRows.Count > 0)
            {
                string testItemId = productQtDataGridView.SelectedRows[0].Cells[(int)eProductQtTestResultList.testItemId].Value.ToString();
                if (testItemId == "8")
                {
                    _absorbeAmountMode = true;
                    OpenProductTestForm("UPDATE");
                    _absorbeAmountMode = false;
                }
            }
        }
        ...
        private void productQtDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex != (int)eProductQtTestResultList.compatibilityOx)
                return;

            if (productQtDataGridView.SelectedRows.Count > 0)
            {
                CompatibilityForm form = new CompatibilityForm(this) { StartPosition = FormStartPosition.Manual, Left = Cursor.Position.X, Top = Cursor.Position.Y };
                form.ShowDialog(); 
            }
        }

        private void productQtDataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex != (int)eProductQtTestResultList.testScore)
                return;

            string testItemId = Utils.GetString(productQtDataGridView.Rows[e.RowIndex].Cells[(int)eProductQtTestResultList.testItemId].Value);
            string testScore = Utils.GetString(productQtDataGridView.Rows[e.RowIndex].Cells[(int)eProductQtTestResultList.testScore].Value);

            string testStandard = Utils.GetString(productQtDataGridView.Rows[e.RowIndex].Cells[(int)eProductQtTestResultList.testStandard].Value);
            string compatibilityOx = "";

            if (testItemId == "3")
            {
                if (testScore.Trim().Split('/').Length == 11)
                {
                    MessageBox.Show("10개의 데이터가 채워졌습니다.");
                    compatibilityOx = testScore.Substring(0, testScore.Length - 1);
                    productQtDataGridView.Rows[e.RowIndex].Cells[(int)eProductQtTestResultList.testScore].Value = compatibilityOx;
            ...
            productQtDataGridView.Rows[e.RowIndex].Cells[(int)eProductQtTestResultList.compatibilityOx].Value = compatibilityOx;
        }
