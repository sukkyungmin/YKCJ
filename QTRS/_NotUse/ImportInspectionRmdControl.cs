...
namespace QTRS
{
    public partial class ImportInspectionRmdControl : UserControl
    {
        private bool _isLoaded = false; 
        public ImportInspectionRmdControl()
        {
            InitializeComponent();
        }

        private void ImportInspectionRmdControl_Load(object sender, EventArgs e)
        {
            InitControls(); 
        }

        private void addImportInspectionRmdButton_Click(object sender, EventArgs e)
        {
            AddImportInspectionRmdForm form = new AddImportInspectionRmdForm(this);
        ...
        {
            SelectImportInspectionRmdList(); 
        }

        private void SelectImportInspectionRmdList()
        {
            ...
            string query = string.Format("EXEC SelectImportInspectionRmdList {0}, '{1}'", productAreaTypeId, componentCode);

            DataSet dataSet = DbHelper.SelectQuery(query);
            if (dataSet == null || dataSet.Tables.Count == 0)
            {
                MessageBox.Show("수입검사 원료 데이터를 가져올 수 없습니다.");
                return;
            }
            ...
            foreach (DataRow dataRow in dataSet.Tables[0].Rows)
            {
                ...
                rmdDataGridView.Rows.Add(false, materialTypeName, warehousingDate, componentCode, itemDesc,maker, lotNo,
                    mainLotNo, baffleReport, sampleOx, ash, wvtr, basicWeight, gravimeter, viscosity,
                    softeningPoint, ph, tensileStrength, thickness, note, idx);
